...
to setup
  clear-all
  set radius (max (list world-width world-height) ) / 2
  set learningRate learning-rate-constant
  set max-iteration 500

  ask patches [
    set weightR random 255
    set weightG random 255
    set weightB random 255
    set pcolor rgb weightR weightG weightB
  ]
  setup-matrix  
  reset-ticks
  tick
...
to setup-matrix
  set matrix []
  ask n-of 5 patches [ set matrix lput  (list (weightR) (weightG) (weightB)) matrix ]
end
...
to go
  ...
  if (ticks > max-iteration or radius <= 1 )[stop]
  let differences 90000000
  let x 0
  let y 0
  let matrix-element one-of matrix

  
  ask patches[
    let d diff self matrix-element 
    if d < differences
    [
      set differences d
      set x pxcor
  ...
  ask patch x y [
      ;set weightR item 0 matrix-element;( weightR + ( item 0 matrix-element - weightR ) / 2 )
      ;set weightG item 1 matrix-element;( weightR + ( item 1 matrix-element - weightR ) / 2 )
      ;set weightB item 2 matrix-element;( weightR + ( item 2 matrix-element - weightR ) / 2 )
      set weightR ( weightR + (distanceFalloff x y) * learningRate * ( item 0 matrix-element - weightR ) )
      set weightG ( weightG + (distanceFalloff x y) * learningRate * ( item 1 matrix-element - weightG ) ) 
      set weightB ( weightB + (distanceFalloff x y) * learningRate * ( item 2 matrix-element - weightB ) )
  ]
  ask patches [
    if distancexy x y < radius
    [
      set weightR ( weightR + (distanceFalloff x y) * learningRate * ( item 0 matrix-element - weightR ) )
      set weightG ( weightG + (distanceFalloff x y) * learningRate * ( item 1 matrix-element - weightG ) ) 
      set weightB ( weightB + (distanceFalloff x y) * learningRate * ( item 2 matrix-element - weightB ) )
  ...
  ;if( radius > exp 1)[
    set radius radius * exp( - ticks * 0.1 / ( max-iteration / ln radius ) )
  ;]
  ;set time time + 1
  if learningRate > 0.0001
  [ set learningRate learning-rate-constant * exp( - ticks / max-iteration )]
  recolor
  ...
  ;; print
  type "iteration " print ticks
  type "radius " print radius
  type "x " type x type " y " print y
  type "learningRate " print learningRate
...
to-report distanceFalloff [ x y ]
  report exp( - (distancexy x y ) / ( 2 * radius * radius ))
end

to-report diff [ p m-e ]
  let result 0
  ask p[
    set result weightR - item 0 m-e + weightG - item 1 m-e + weightB - item 2 m-e
  ]
  report result
end
...
to recolor
  ask patches [ set pcolor rgb weightR weightG weightB ]
end
